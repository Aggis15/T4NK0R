...
class Notifications(commands.Cog):
    ...
    async def notifications(
        self, ctx, value: Option(str, "Make your choice!", choices=["on", "off"])
    ):
        ...
        if value == "off":
            await conn.execute(
                f"UPDATE {TABLE_NAME} SET doNotify = false WHERE userid = {ctx.author.id}"
            )
        else:
            await conn.execute(
                f"UPDATE {TABLE_NAME} SET doNotify = true WHERE userid = {ctx.author.id}"
            )
