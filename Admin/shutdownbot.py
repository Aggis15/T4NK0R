...
class rebootBot(commands.Cog):
    ...
    async def shutdownbot(self, ctx):
        await ctx.respond(
            f"{warningTSE} This is a dangerous command to execute. Please be aware that if you use this, you must manually turn the bot on again. If you are sure about this, type '**Yes, I agree to killing the bot!**'. You have 20 seconds, starting now.{warningTSE}"
        )
        ...
        verification = await self.bot.wait_for(
            "message", check=check(ctx.author), timeout=20
        )
