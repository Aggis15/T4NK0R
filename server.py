...
def twitchPost():
    ...
    if (
        ...
        in request.headers["Twitch-Eventsub-Message-Type"]
    ):
        ...
        if signature == twitchSignature:
            ...
            req = r.get(
                f"https://api.twitch.tv/helix/streams?user_id={userID}", headers=headers
            )
            if req.status_code == 401:
                ...
                req = r.get(
                    f"https://api.twitch.tv/helix/streams?user_id={userID}",
                    headers=headers,
                )
            ...
            webhook = DiscordWebhook(url=WEBHOOK_URL, content="<@&958761720963301418>")
